...
本轮针对真实单屏记录补齐了最低保持时间、设备就绪重试和异常收尾预算，最终 Release 包已重新构建并替换固定安装路径。代码工作分支为 `codex/fix-recovery-timing`。

- `RecoveryTimeouts` 对旧配置缺失字段使用兼容默认值：单屏观察 10 秒、ANT 断电最低保持 10 秒、ANT 复电稳定 15 秒、MSI 模式稳定 15 秒、USB/HID 就绪等待 15 秒；读检测仍按 0.5 秒轮询。
- ANT 断电后同时观察 ANT 和 MSI，15 秒期限到才启动复电兜底；复电后再稳定观察 15 秒。MSI 1080P 阶段的双屏窗口从切模开始计算，包含确认与 15 秒保持；UHD 收尾在写入后保持 15 秒，再做连续 10 秒双屏、HID UHD、系统 4K 检查。
- USB 接口暂时消失时在独立 15 秒窗口重试；供电收尾最多 30 秒、模式收尾最多 45 秒。未实际写入时日志显示“额度未使用”，不会把一次读不到设备误记为已消耗额度。
- 模拟设备已覆盖第 10～15 秒上线、ANT 枚举残留、HID 暂时消失、永久失联、写入应答丢失、取消和重启接续；核心 58 项与平台 15 项测试均通过，共 73 项、0 失败。平台测试结果保存在 `.build/timing-repair-20260908/platform-tests-final.log`。
- Release 构建和签名校验通过。最终 CLI SHA-256 为 `f0272bbf262457a62ebd4a82addb129ce4770adf33254694010dbcd55d036b94`，App 可执行文件 SHA-256 为 `77187a6268e9304fe70ba660c072e869f157af71ac4babdf8dddf4677e8c5b05`；`/Applications/DisplayRecoveryAutomation.app` 与 `dist/` 产物一致。
- 固定安装包当前只有一个运行实例，登录项数量为 1，自动恢复配置已开启。用户配置保留原目标绑定、凭据文件和事务记录；当前事务仍保留 `MSI 4K 待恢复=true`，因为物理 MSI HID 尚未枚举。

本次真实单屏故障复现记录：19:24:15 发送 ANT 断电，19:24:30 等满 15 秒后启动复电，19:24:45 完成 15 秒复电观察；期间系统始终只枚举 ANT，MSI HID 在独立 15 秒窗口内仍不可用。程序随后安全停止，未写入 MSI 4K，日志明确记录“收尾写入额度未使用”。手动入口确实接续了旧事务，并在收尾后按授权重新执行了一次完整 ANT 断电/复电流程；由于 MSI 物理接口缺失，最终不能声称双屏恢复成功。待 MSI 重新枚举后，下一次手动入口会继续使用保留的目标身份和 4K 责任，仍受三次失败上限与单次收尾额度保护。
