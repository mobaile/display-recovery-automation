...
parser.add_argument("--qr", action="store_true", help="Directly use QR code login")
parser.add_argument("--auto-save", action="store_true", help="Automatically save plug token to macOS Keychain and config")
...
def present_image_image(
    ...
    qr_path = "/tmp/xiaomi_login_qr.jpg"
    try:
        with open(qr_path, "wb") as f:
            f.write(image_content)
        print_if_interactive(f"{Fore.GREEN}登录二维码图片已保存至: {qr_path}")
        if sys.platform == "darwin":
            import subprocess
            subprocess.run(["open", qr_path])
            print_if_interactive(f"{Fore.GREEN}已通过系统预览（Preview）自动弹出二维码窗口，请使用手机米家 App 扫一扫！")
    except Exception as e:
        _LOGGER.debug(e)
...
def main() -> None:
            ...
            for home in all_homes:
                ...
                if devices is not None:
                    ...
                    for device in devices["result"]["device_info"]:
                        ...
                        if args.auto_save and "token" in device and device["token"]:
                            tok = device["token"]
                            if tok != "0" * 32 and tok != "f" * 32:
                                if device.get("localip") == "192.168.31.125" or "212a01" in device.get("model", ""):
                                    print_tabbed(f"{Fore.GREEN}==> 发现目标插座！正在自动存入系统 Keychain...", 3)
                                    import subprocess
                                    subprocess.run([
                                        "security", "add-generic-password", "-U",
                                        "-s", "local.codex.display-recovery-automation",
                                        "-a", "miot-token",
                                        "-w", tok
                                    ])
                                    cfg_dir = os.path.expanduser("~/Library/Application Support/DisplayRecoveryAutomation")
                                    os.makedirs(cfg_dir, exist_ok=True)
                                    cfg_path = os.path.join(cfg_dir, "config.json")
                                    cfg = {
                                        "plug": {
                                            "host": device.get("localip", "192.168.31.125"),
                                            "model": device.get("model", "chuangmi.plug.212a01"),
                                            "port": 54321
                                        },
                                        "recovery": {
                                            "automaticRecoveryEnabled": False,
                                            "pollInterval": 0.5,
                                            "recoveryCooldown": 30.0,
                                            "roles": {
                                                "modeSwitch": None,
                                                "powerControlled": None
                                            },
                                            "safeMode": {
                                                "height": 1080,
                                                "refreshRate": 320,
                                            ...
                                            "timeouts": {
                                                "newDisplayOnline": 20.0,
                                                "oldDisplayOnline": 30.0,
                                                "powerOff": 15.0,
                                                "powerOn": 15.0,
                                                "restoreMode": 30.0,
                                                "safeMode": 20.0
                                    ...
                                    print_tabbed(f"{Fore.GREEN}==> 配置已保存至 {cfg_path}", 3)
                    print_tabbed(f"{Fore.BLUE}---------", 3)
                    print_if_interactive()
                else:
    ...
    if not args.non_interactive and not args.qr and not args.auto_save:
        print_if_interactive()
